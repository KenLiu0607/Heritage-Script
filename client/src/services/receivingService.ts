...
export const receivingService = {
  ...
  calculateSummary(items: ReceiptItem[]): ReceiptSummary {
    ...
    const frozenWeight = items.filter(r => r.type === 'frozen').reduce((acc, curr) => acc + curr.weight, 0);
    const chilledWeight = items.filter(r => r.type === 'chilled').reduce((acc, curr) => acc + curr.weight, 0);
    ...
    return {
      ...
      frozenWeight,
      chilledWeight,
      frozenPercentage: totalWeight > 0 ? Math.round((frozenWeight / totalWeight) * 100) : 0,
      chilledPercentage: totalWeight > 0 ? Math.round((chilledWeight / totalWeight) * 100) : 0,
    };
  }
};
