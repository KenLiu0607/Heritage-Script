      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card className="hover:shadow-md transition-shadow">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">總產量 (本月)</CardTitle>
            <Package className="h-4 w-4 text-primary" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold font-mono">12,345 <span className="text-sm font-normal text-muted-foreground">kg</span></div>
        ...
        <Card className="hover:shadow-md transition-shadow">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">存活率 (平均)</CardTitle>
        ...
        <Card className="hover:shadow-md transition-shadow border-l-4 border-l-amber-500">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">庫存預警</CardTitle>
            <AlertTriangle className="h-4 w-4 text-amber-500" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold font-mono text-amber-600">3 項</div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-7">
            ...
            <div className="h-[300px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={data} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                  ...
                  <Area type="monotone" dataKey="production" stroke="hsl(var(--primary))" fillOpacity={1} fill="url(#colorProd)" name="生產量" />
                  <Area type="monotone" dataKey="sales" stroke="hsl(var(--chart-2))" fillOpacity={1} fill="url(#colorSales)" name="銷售量" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
        ...
        <Card className="col-span-3">
          ...
          <CardContent>
            <div className="h-[300px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={fcrData} layout="vertical" margin={{ top: 0, right: 30, left: 20, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" horizontal={true} vertical={false} stroke="hsl(var(--border))" />
                  <XAxis type="number" domain={[0, 2]} hide />
                  <YAxis dataKey="batch" type="category" stroke="#888888" fontSize={12} tickLine={false} axisLine={false} width={50} />
                  <Tooltip 
                    cursor={{fill: 'hsl(var(--muted))'}}
                    contentStyle={{ backgroundColor: 'hsl(var(--card))', borderColor: 'hsl(var(--border))', borderRadius: 'var(--radius)' }}
                  />
                  <Bar dataKey="fcr" fill="hsl(var(--primary))" radius={[0, 4, 4, 0]} barSize={20} name="FCR" />
                </BarChart>
              </ResponsiveContainer>
            </div>
            ...
            <ul className="space-y-4">
              {[
                ...
                <li key={i} className="flex items-center gap-3 text-sm">
                  <div className={cn("w-2 h-2 rounded-full", item.urgent ? "bg-red-500" : "bg-blue-500")} />
                  <span className={cn(item.urgent && "font-medium")}>{item.text}</span>
                </li>
              ))}
            </ul>
          </CardContent>
          ...
          <CardContent>
             <div className="space-y-4 text-sm text-muted-foreground">
                <div className="p-3 bg-muted rounded-md border border-border">
                  ...
                  <p>系統將於本週日凌晨 2:00 進行例行維護，預計暫停服務 2 小時。</p>
                </div>
                <div className="p-3 bg-muted rounded-md border border-border">
